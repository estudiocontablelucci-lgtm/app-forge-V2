...
export async function invitar({ ownerUserId, email, nombreCoach }) {
  ...
  // No se puede invitar a alguien que ya es alumno activo.
  const yaEs = await db.execute({
    sql: `SELECT 1 FROM coach_athletes ca JOIN users u ON u.id = ca.athlete_id
          WHERE ca.coach_id = ? AND u.email = ? AND ca.status IN ('active','invited') LIMIT 1`,
    args: [coach.id, mail],
  });
  if (yaEs.rows.length) return { ok: false, motivo: "ya-es-alumno" };
...
export async function invitacionesPara(email) {
  const r = await getDb().execute({
    ...
    args: [String(email || "").toLowerCase().trim(), now()],
  });
  return r.rows.map((i) => ({
...
export async function aceptarInvitacion({ token, userId, email, politica = "v1" }) {
  ...
  // El link es de un email concreto: no sirve para que entre otra persona.
  if (email && String(email).toLowerCase().trim() !== inv.email) return { ok: false, motivo: "otro-email" };
...
export async function listarAlumnos(coachId) {
  const r = await getDb().execute({
    sql: `SELECT ca.athlete_id, ca.status, ca.accepted_at, u.display_name, u.email
