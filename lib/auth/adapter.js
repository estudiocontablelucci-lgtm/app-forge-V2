...
export function ForgeAdapter() {
  ...
  /**
   * Resuelve la cuenta por CASILLA y no por grafia.
   *
   * Es el unico punto por el que NextAuth pregunta "quien es este email": lo usa
   * el magic link y tambien `allowDangerousEmailAccountLinking` para enganchar
   * el login de Google con la cuenta que ya existe. Comparando texto exacto,
   * entrar una vez con puntos y otra sin puntos creaba dos cuentas.
   *
   * El fallback exacto es para las filas anteriores al backfill: sin el, un
   * usuario que ya existia no se encontraria y entraria a una cuenta nueva vacia.
   */
  const buscarPorCasilla = async (email) =>
    (await getUserBy("email_canon", canonicalizarEmail(email)))
    || (await getUserBy("email", normalizarEmail(email)));

  return {
    async createUser(user) {
      const email = normalizarEmail(user.email);

      // Se pregunta primero: NextAuth llama a createUser cuando getUserByEmail
      // no encontro nada, pero entre las dos llamadas puede haber una fila con
      // la misma casilla escrita distinto que el indice de `email` no ve.
      const yaEsta = await buscarPorCasilla(email);
      if (yaEsta) return yaEsta;

      const ts = now();
      const id = uid();
      await db.execute({
        sql: `INSERT INTO users (id, email, email_canon, display_name, image, email_verified, role, active, created_at, updated_at)
              VALUES (?, ?, ?, ?, ?, ?, 'athlete', 1, ?, ?)
              ON CONFLICT (email) DO NOTHING`,
        args: [
          id, email, canonicalizarEmail(email),
      ...
      return buscarPorCasilla(email);
    },

    getUser: (id) => getUserBy("id", id),

    getUserByEmail: (email) => buscarPorCasilla(email),
