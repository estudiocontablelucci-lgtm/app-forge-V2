...
/**
 * Programas cuyas referencias al catalogo EXISTEN de verdad.
 *
 * `program_exercises.exercise_id` es una FK a `exercises`. Un ejercicio que
 * apunta a una entrada que no esta en el catalogo hace fallar el INSERT ENTERO
 * del programa con `FOREIGN KEY constraint failed`, y como el push se reintenta
 * en cada sincronizacion, **falla para siempre**. Paso en produccion: 16 de 36
 * ejercicios apuntaban a entradas inexistentes y el programa no subio nunca,
 * mientras la app anunciaba "Sincronizado".
 *
 * La referencia se resuelve en dos pasos, en este orden:
 *
 * 1. **Por NOMBRE.** Si el catalogo tiene ese ejercicio con otro id, se repunta
 *    ahi. Es lo correcto: el ejercicio existe, lo que estaba mal era el id.
 * 2. **Se suelta.** Si no hay a quien apuntar, el `exerciseId` se va a null. El
 *    programa sigue funcionando —`name` viaja denormalizado al lado— y el push
 *    entra. Una referencia rota no vale mas que ninguna, y bloquear la
 *    sincronizacion de todo el programa es un precio que nadie eligio pagar.
 *
 * Esto NO reemplaza a `absorberDeProgramas`, que crea las entradas que faltan:
 * corre despues, sobre lo que aquella no pudo resolver.
 */
export function sinReferenciasHuerfanas(programs, catalog) {
  const porId = new Set((catalog || []).map((c) => c.id));
  const porNombre = new Map((catalog || []).map((c) => [normalizar(c.name), c.id]));
  let sueltas = 0;
  let repuntadas = 0;

  const limpios = (programs || []).map((p) => {
    let toco = false;
    const exercises = (p.exercises || []).map((e) => {
      if (!e.exerciseId || porId.has(e.exerciseId)) return e;
      const porNom = porNombre.get(normalizar(e.name));
      toco = true;
      if (porNom) { repuntadas++; return { ...e, exerciseId: porNom }; }
      sueltas++;
      return { ...e, exerciseId: null };
    });
    return toco ? { ...p, exercises } : p;
  });

  return { programs: limpios, sueltas, repuntadas };
}
