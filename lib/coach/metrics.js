 ...
 */
import { brzycki, isNum, refFor, cargaEfectiva } from "../formulas.js";
import { pesoVigente } from "../medidas.js";
...
/** Lo que dice el registro, sin mas. El comportamiento de siempre. */
export const kgCrudo = (s) => Number(s.kg);

/**
 * Los kilos que movio el alumno, con el peso corporal incluido donde
 * corresponde.
 *
 * Existe para que el entrenador y el alumno vean EL MISMO numero: la pantalla
 * de Progreso ya suma el peso corporal en dominadas y fondos, y si la ficha no
 * lo hiciera, los dos estarian mirando e1RM distintos del mismo ejercicio.
 *
 * `medidas` son las tomas del alumno y `fechaDe` resuelve cuando se entreno esa
 * semana y sesion: el peso que entra es el VIGENTE ENTONCES, no el de hoy.
 */
export function cargaConPeso(medidas = [], fechaDe = () => null) {
  return (s, ejercicios = []) => {
    const ex = ejercicios.find((e) => e.id === (s.programExerciseId || s.exerciseName));
    return cargaEfectiva({
      ref: refFor(ex, s.week),
      kg: s.kg,
      pesoCorporal: pesoVigente(medidas, fechaDe(s.week, s.sessionCode ?? s.session) ?? Date.now()),
    });
  };
}
 ...
 * pasos no suman tonelaje. Los de peso corporal SI suman —levantarse a uno
 * mismo es carga— siempre que haya una medicion con la que saber cuanto pesaba
 * el alumno ese dia; sin ella quedan afuera, como quedaban siempre.
 */
export function tonelajePorSemana(sets, ejercicios = [], carga = kgCrudo) {
  const enPasos = new Set(ejercicios.filter((e) => e.unit === "pasos").map((e) => e.id));
  const out = {};
  for (const s of sets) {
    if (enPasos.has(s.programExerciseId)) continue;
    const kg = carga(s, ejercicios), reps = parseInt(s.reps, 10);
...
export function e1rmPorEjercicio(sets, ejercicios = [], carga = kgCrudo) {
  const enPasos = new Set(ejercicios.filter((e) => e.unit === "pasos").map((e) => e.id));
  const out = {};
  for (const s of sets) {
    const clave = s.programExerciseId || s.exerciseName;
 ...
 * hubiera entrenado.
 */
export function fichaDeAlumno({ programa, sesiones = [], sets = [], medidas = [], ahora = Date.now() }) {
  ...
  // Sin medidas del alumno, `cargaConPeso` deja los ejercicios a peso corporal
  // afuera igual que antes: no inventa un peso.
  const fechas = new Map(sesiones.map((s) => [`${s.week}|${s.session ?? s.sessionCode}`, s.date]));
  const carga = cargaConPeso(medidas, (w, sess) => fechas.get(`${w}|${sess}`));
  const tonnage = tonelajePorSemana(sets, ejercicios, carga);
  ...
  return {
    ...
    e1rm: e1rmPorEjercicio(sets, ejercicios, carga),
