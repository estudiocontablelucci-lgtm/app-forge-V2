...
/**
 * Entrada del catalogo base, igual en toda instalacion (`base-press-plano-barra`).
 *
 * Estos ids NO se prefijan: son universales a proposito. Si cada usuario
 * subiera su propio `agustin~base-dominadas`, el mismo ejercicio de siempre
 * seria uno distinto por persona y no se podria comparar nada entre cuentas.
 * Los ids locales salen de `uid()` (base36 sin guiones), asi que ninguno puede
 * empezar con "base-" por accidente.
 */
export const esDeCatalogoBase = (id) => typeof id === "string" && id.startsWith("base-");
...
export function scope(userId, localId) {
  ...
  if (esRemoto(localId) || esDeCatalogoBase(localId)) return localId;
...
export function scopeProgram(userId, program) {
  return {
    ...
    exercises: (program.exercises || []).map((e) => ({
      ...
      // La referencia al catalogo viaja con el ejercicio. Sin esto el programa
      // llegaba al servidor sin saber QUE ejercicio es, solo con su nombre.
      exerciseId: e.exerciseId ? scope(userId, e.exerciseId) : null,
    })),
  };
}
...
export function unscopeProgram(userId, program) {
  return {
    ...
    exercises: (program.exercises || []).map((e) => ({
      ...
      exerciseId: e.exerciseId ? unscope(userId, e.exerciseId) : null,
    })),
  };
}

/** Entradas del catalogo, con sus ids llevados a remoto y de vuelta. */
export function scopeCatalog(userId, catalog) {
  return (catalog || []).map((c) => ({ ...c, id: scope(userId, c.id) }));
}

export function unscopeCatalog(userId, catalog) {
  return (catalog || []).map((c) => ({ ...c, id: unscope(userId, c.id) }));
}
