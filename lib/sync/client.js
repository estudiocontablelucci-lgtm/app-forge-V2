...
/**
 * ¿Se llega al servidor ahora mismo?
 *
 * Con reloj, porque sin senal `fetch` no falla rapido: el sistema tarda en
 * darse por vencido y mientras tanto la app afirmaba tener conexion. Es la
 * misma razon por la que la navegacion del service worker tiene su propio
 * reloj.
 *
 * Si `navigator.onLine` dice que NO hay interfaz de red, eso ya es definitivo y
 * no hace falta preguntar: los falsos negativos no existen, los falsos
 * positivos si.
 */
export async function hayServidor(ms = 2500) {
  if (typeof navigator !== "undefined" && navigator.onLine === false) return false;
  const corte = new AbortController();
  const reloj = setTimeout(() => corte.abort(), ms);
  try {
    const r = await fetch("/api/ping", { cache: "no-store", signal: corte.signal });
    return r.ok;
  } catch {
    return false;
  } finally {
...
export async function pullAll() {
  // Con reloj: es una LECTURA, abandonarla no pierde nada, y sin senal quedaba
  // colgada dejando "Sincronizando…" en pantalla un rato largo. Los pushes no
  // lo llevan a proposito: abandonar una escritura que el servidor quiza acepto
  // es otra cosa.
  const corte = new AbortController();
  const reloj = setTimeout(() => corte.abort(), 10000);
  try {
    const r = await fetch("/api/sync", { signal: corte.signal });
    if (r.status === 401) return { ok: false, motivo: "sin-sesion" };
    if (!r.ok) return { ok: false, motivo: `http-${r.status}` };
    return { ok: true, data: await r.json() };
  } catch {
